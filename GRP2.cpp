...
int main()
{
	...
	model->save("LBPHface.xml");//将训练好的人脸PCA模型保存成XML文件
	//model->load("LBPHface.xml");//从保存的XML文件中读取PCA模型
	model->set("threshold", 80);//LBPH阈值设置为80查不多不会误检测；70时脸必须是正脸否则识别不了
...
//按键‘b’，开始采集人脸；按键‘t’，暂停采集人脸；按键‘f’，此人人脸采集结束，按键‘q’，退出采集人脸函数，表示所有人脸样本采集完毕
void takphoto()
{
